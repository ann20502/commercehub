package com.commercehub.etl.detail.usecaseinteractor.order.transformer;

import com.commercehub.etl.core.entity.order.OrderShopee;
import com.commercehub.etl.core.entitybuilder.order.shopee.*;
...
public class OrderShopeeTransformer {

    public static OrderShopee from(GetOrderDetailOutput.Order order, GetEscrowDetailOutput escrow) {
        OrderShopee.RecipientAddress address = getAddress(order.getRecipient_address());
        List<OrderShopee.Item> items = getItems(order.getItem_list());
        OrderShopee.Invoice invoice = order.getInvoice_data() == null ? null : getInvoice(order.getInvoice_data());
        OrderShopee.Escrow finalEscrow = getEscrow(escrow.getResponse().getOrder_income());

        OrderShopee result = new OrderShopeeBuilder()
    ...
    private static OrderShopee.RecipientAddress getAddress(GetOrderDetailOutput.RecipientAddress addr) {
    ...
    private static List<OrderShopee.Item> getItems(List<GetOrderDetailOutput.Item> items) {
        return items.stream()
                .map(item -> new ItemBuilder()
                        ...
                        .setImageInfo(new OrderShopee.ImageInfo(item.getImage_info().getImage_url()))
                        .createItem())
                .collect(Collectors.toList());
    }

    private static OrderShopee.Invoice getInvoice(GetOrderDetailOutput.Invoice invoice) {
    ...
    private static OrderShopee.Escrow getEscrow(GetEscrowDetailOutput.OrderIncome income) {
